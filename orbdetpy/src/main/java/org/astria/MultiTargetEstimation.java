...
public final class MultiTargetEstimation {
	...
	public MultiTargetEstimation(ArrayList<Settings> cfgList, ArrayList<Measurements> obsList) {
		...
		singleObject = (measNames[0] != Settings.MeasurementType.RANGE
				&& measNames[0] != Settings.MeasurementType.RANGE_RATE) || measNames.length == 1;
	}
	...
	private class MultiTargetFilter {
		...
		private void determineOrbit() {
			...
			for (int smoothIter = 0; smoothIter < odCfg.estmSmootherIterations; smoothIter++) {
				...
				for (int measIndex = 0,
						additionalMeas = 0; measIndex < odObs.array.length; measIndex += additionalMeas + 1) {
					additionalMeas = 0;
					...
					while (measIndex + additionalMeas + 1 < odObs.array.length
							&& odObs.array[measIndex + additionalMeas]
									.equals(odObs.array[measIndex + additionalMeas + 1]))
						additionalMeas++;
					...
					if (activateCAR && rsoChoices.size() == 0) {
						for (int measNum = 0; measNum <= additionalMeas; measNum++) {
							...
							if (singleObject)
								newHypotheses = generateOpticalHypotheses(odObs.array[measIndex + measNum],
										R.getEntry(0, 0), 1E-10, R.getEntry(1, 1), 1E-10);
							else
								newHypotheses = generateRadarHypotheses(odObs.array[measIndex + measNum],
										R.getEntry(0, 0), R.getEntry(1, 1), 15E-6, 15E-6);
					...
					for (int objNum = 0; objNum < rsoChoices.size(); objNum++) {
						for (Iterator<SpaceObject> iter = rsoChoices.get(objNum).iterator(); iter.hasNext();) {
							...
							if (currSC.dataAssociated) {
								...
								for (int j = 6; j < odCfg.parameters.size() + 6; j++) {
									...
									for (int i = 0; i < numSigmas; i++)
										sigData[j][i] = FastMath.min(FastMath.max(sigData[j][i], tempep.min),
												tempep.max);
								}
							...
							if (FastMath.abs(stepEnd - stepStart) > 1.0E-6) {
								try {
									propagator.propagate(stepStart, currSC.sigma, stepEnd, currSC.propSigma,
											measIndex > 0);
							...
							currSC.marginalEvents.add(new JPDALikelihood(objNum, 0,
									(1 - odCfg.estmDetectionProbability) * currSC.hypothesisWeight));
							for (int measNum = 0; measNum <= additionalMeas; measNum++) {
								...
								RealMatrix maha = mahaTemp.transpose()
										.multiply(MatrixUtils.inverse(Pyy).multiply(mahaTemp));
								if (Math.sqrt(maha.getEntry(0, 0)) < odCfg.estmGatingThreshold)
									currSC.marginalEvents.add(new JPDALikelihood(objNum, measNum + 1,
											(1 - new ChiSquaredDistribution(Rsize)
													.cumulativeProbability(maha.getEntry(0, 0)))
													* currSC.hypothesisWeight));
							}
						}
					}

					// JPDA
					double[][] sumJPDALikelihood = new double[rsoChoices.size()][additionalMeas + 2]; // +1 to account
																										// for
																										// measurement 0
																										// case
					for (int objNum = 0; objNum < rsoChoices.size(); objNum++) {
					...
					double[][] betaSatMeas = new double[rsoChoices.size()][additionalMeas + 2]; // +1 to account for
																								// measurement 0 case
					// Compute beta based on the object & measurement pair
					for (int i = 0; i < jointEvents.size(); i++) {
						for (int j = 0; j < jointEvents.get(i).size(); j++)
							betaSatMeas[jointEvents.get(i).get(j).object][jointEvents.get(i)
									.get(j).measurement] += JPDAProbability[i];
					}
						...
						for (int hypNum = 0; hypNum < rsoChoices.get(objNum).size(); hypNum++) {
							...
							for (int i = 0; i < currSC.marginalEvents.size(); i++) {
								if (currSC.marginalEvents.get(i).measurement > 0) {
									double betaTemp = 0;
									RealVector rawMeas = updatePrep(currSC, tm,
											measIndex + currSC.marginalEvents.get(i).measurement - 1);
									RealVector yhatpre = addColumns(currSC.estimMeas).mapMultiplyToSelf(weight);
							...
							for (int i = 0; i < numSigmas; i++) {
								RealVector y = currSC.estimMeas.getColumnVector(i).subtract(yhatpre);
								Pyy = Pyy.add(y.outerProduct(y).scalarMultiply(weight));
								Pxy = Pxy.add(currSC.propSigma.getColumnVector(i).subtract(currSC.xhatPrev)
										.outerProduct(y).scalarMultiply(weight));
							}

							RealMatrix K = Pxy.multiply(MatrixUtils.inverse(Pyy));
							RealMatrix Ptilda = K
									.multiply((betavvt.subtract(betav.outerProduct(betav))).multiply(K.transpose()));
							currSC.xhat = currSC.xhatPrev.add(odCfg.parameterMatrix.multiply(K).operate(betav));
							currSC.P = currSC.Pprop.subtract(odCfg.parameterMatrix
									.multiply(K.multiply(Pyy.multiply(K.transpose()))).subtract(Ptilda));

							// For estimated parameters, return xhat values to max/min bounds. If far
							// outside the bounds, can result in
							// singular matrix when taking the inverse for smoothing.
							for (int j = 6; j < odCfg.parameters.size() + 6; j++) {
								Settings.Parameter tempep = odCfg.parameters.get(j - 6);
								currSC.xhat.setEntry(j,
										FastMath.min(FastMath.max(currSC.xhat.getEntry(j), tempep.min), tempep.max));
							}

							// If data associated, save data and compute smoother values.
							if (currSC.dataAssociated) {
								double[] pv = currSC.xhat.toArray();
								CartesianOrbit orb = new CartesianOrbit(
										new PVCoordinates(new Vector3D(pv[0], pv[1], pv[2]),
								...
								Estimation.EstimationOutput odout = new Estimation.EstimationOutput(
										odObs.array[measIndex].time, odObs.array[measIndex].station);
								...
								if (singleObject)
									odout.postFit = odObs.array[measIndex].helpers[0].estimate(0, 0, ssta)
											.getEstimatedValue();
								else
									odout.postFit = new double[]{
											odObs.array[measIndex].helpers[0].estimate(0, 0, ssta)
													.getEstimatedValue()[0],
											odObs.array[measIndex].helpers[1].estimate(0, 0, ssta)
													.getEstimatedValue()[0]};
					...
					for (int hypNum = 0; hypNum < rsoList.size(); hypNum++) {
						...
						for (int i = 0; i < smSize; i++) {
							SmootherTimeStep smDatak1 = currSC.smootherData.get(smSize - i),
									smDatak = currSC.smootherData.get(smSize - i - 1);
							...
							for (int j = 0; j < numSigmas; j++)
								Csmoother = Csmoother.add(smDatak.sigPost.getColumnMatrix(j).subtract(smDatak.xpost)
										.multiply(
												smDatak1.sigPre.getColumnMatrix(j).subtract(smDatak1.xpre).transpose())
										.scalarMultiply(weight));
							Asmoother = Csmoother.multiply(MatrixUtils.inverse(smDatak1.Ppre));
							smDatak.xstar = smDatak.xpost
									.add(Asmoother.multiply(smDatak1.xstar.subtract(smDatak1.xpre)));
							smDatak.Pstar = smDatak.Ppost.add(Asmoother.multiply(
									(smDatak1.Pstar.subtract(smDatak1.Ppre)).multiply(Asmoother.transpose())));
						...
						if (!rsoList.get(0).fromCAR) {
							for (int i = 0; i < currSC.smootherData.size() - 1; i++) {
								...
								RealMatrix delx = smDatak.xpost.subtract(smDatak.xstar),
										delP = smDatak.Ppost.subtract(smDatak.Pstar);
								for (int j = 0; j < 5; j++) {
							...
							for (int j = 0; j < currSC.smootherData.size(); j++) {
								...
								sms[0] = new SpacecraftState(new CartesianOrbit(
										new PVCoordinates(new Vector3D(pv[0], pv[1], pv[2]),
												new Vector3D(pv[3], pv[4], pv[5])),
										odCfg.propInertialFrame, tm, Constants.EGM96_EARTH_MU));
								currSC.estOutput.get(j).estimatedState = currSC.smootherData.get(j).xstar.getColumn(0);
								currSC.estOutput.get(j).estimatedCovariance = getLowerTriangle(
										currSC.smootherData.get(j).Pstar);
								// Compute smoothed residuals
								if (singleObject)
									currSC.estOutput.get(j).postFit = currSC.smootherData.get(j).measObjs[0]
											.estimate(0, 0, sms).getEstimatedValue();
								else {
									currSC.estOutput.get(j).postFit = new double[]{
											currSC.smootherData.get(j).measObjs[0].estimate(0, 0, sms)
													.getEstimatedValue()[0],
											currSC.smootherData.get(j).measObjs[1].estimate(0, 0, sms)
													.getEstimatedValue()[0]};
								}
							}
						} else {
		...
		private ArrayList<Hypothesis> generateOpticalHypotheses(Measurements.Measurement obs, double sigmaRA,
				double sigmaRAd, double sigmaDec, double sigmaDecd) {
			double[] x0 = odCfg.getInitialState();
			double RA = obs.values[0], RA_d = obs.angleRates[0], Dec = obs.values[1], Dec_d = obs.angleRates[1];
			TimeStampedPVCoordinates station = odCfg.stations.get(obs.station).getBaseFrame().getPVCoordinates(obs.time,
					odCfg.propInertialFrame);
			...
			ArrayList<CAR.CARGaussianElement> CARGaussians = new CAR(RA, Dec, RA_d, Dec_d, station, 100000.0, 100.0,
					10000.0, 17000000, 37000000, 0.05, singleObject).getCAR();
			for (int i = 0; i < CARGaussians.size(); i++) {
				if (CARGaussians.get(i).ordinateStd < 0)
					continue;
				meanTemp.setEntry(0, CARGaussians.get(i).abscissaMean);
				meanTemp.setEntry(1, RA);
				meanTemp.setEntry(2, Dec);
				meanTemp.setEntry(3, CARGaussians.get(i).ordinateMean);
				meanTemp.setEntry(4, RA_d);
				meanTemp.setEntry(5, Dec_d);
				covarTemp.setEntry(0, 0, Math.pow(CARGaussians.get(i).abscissaStd, 2));
				covarTemp.setEntry(1, 1, sigmaRA);
				covarTemp.setEntry(2, 2, sigmaDec);
				covarTemp.setEntry(3, 3, Math.pow(CARGaussians.get(i).ordinateStd, 2));
				covarTemp.setEntry(4, 4, sigmaRAd);
				covarTemp.setEntry(5, 5, sigmaDecd);
		...
		private ArrayList<Hypothesis> generateRadarHypotheses(Measurements.Measurement obs, double sigmaRange,
				double sigmaRR, double sigmaRA, double sigmaDec) {
			double[] x0 = odCfg.getInitialState();
			...
			ArrayList<CAR.CARGaussianElement> CARGaussians = new CAR(ra, dec, range, rangeRate, station, 5E-6, 5E-6,
					5E-6, 17000000, 37000000, 0.1, singleObject).getCAR();
			for (int i = 0; i < CARGaussians.size(); i++) {
				if (CARGaussians.get(i).ordinateStd < 0)
					continue;
				meanTemp.setEntry(0, range);
				meanTemp.setEntry(1, ra);
				meanTemp.setEntry(2, dec);
				meanTemp.setEntry(3, rangeRate);
				meanTemp.setEntry(4, CARGaussians.get(i).abscissaMean);
				meanTemp.setEntry(5, CARGaussians.get(i).ordinateMean);
				covarTemp.setEntry(0, 0, sigmaRange);
				covarTemp.setEntry(1, 1, sigmaRA);
				covarTemp.setEntry(2, 2, sigmaDec);
				covarTemp.setEntry(3, 3, sigmaRR);
				covarTemp.setEntry(4, 4, Math.pow(CARGaussians.get(i).abscissaStd, 2));
			...
			for (int hyp1 = 0; hyp1 < hypotheses.size(); hyp1++) {
				...
				for (Iterator<SpaceObject> iter2 = hypotheses.iterator(); iter2.hasNext();) {
					...
					if (Math.min(maha1.getEntry(0, 0), maha2.getEntry(0, 0)) < 1) {
						RealVector xhatTemp = obj1.xhat.mapMultiply(obj1.hypothesisWeight)
								.add(obj2.xhat.mapMultiply(obj2.hypothesisWeight))
						...
						RealMatrix PTemp = obj1.P.add(matrixTemp.multiply(matrixTemp.transpose())).scalarMultiply(
								obj1.hypothesisWeight / (obj1.hypothesisWeight + obj2.hypothesisWeight));
						matrixTemp = MatrixUtils.createColumnRealMatrix(obj2.xhat.toArray());
						PTemp = PTemp.add(obj2.P.add(matrixTemp.multiply(matrixTemp.transpose())).scalarMultiply(
								obj2.hypothesisWeight / (obj1.hypothesisWeight + obj2.hypothesisWeight)));
		...
		private Array2DRowRealMatrix generateSigmaPoints(RealVector mean, RealMatrix cov) {
			...
			RealMatrix sqrP = new CholeskyDecomposition(Ptemp.add(Ptemp.transpose()).scalarMultiply(0.5), 1E-6, 1E-16)
					.getL();
		...
		private double[] rangeRaDecToCart(RealVector rRaDec, AbsoluteDate date, String stat) {
			...
			Vector3D topoPos = new Vector3D(new double[]{Range * Math.cos(Dec) * Math.cos(RA),
					Range * Math.cos(Dec) * Math.sin(RA), Range * Math.sin(Dec)});
			Vector3D topoVel = new Vector3D(new double[]{
					Range_d * Math.cos(Dec) * Math.cos(RA) - Range * Math.sin(Dec) * Math.cos(RA) * Dec_d
							- Range * Math.cos(Dec) * Math.sin(RA) * RA_d,
					Range_d * Math.cos(Dec) * Math.sin(RA) - Range * Math.sin(Dec) * Math.sin(RA) * Dec_d
							+ Range * Math.cos(Dec) * Math.cos(RA) * RA_d,
		...
		private ArrayList<ArrayList<JPDALikelihood>> JPDAJointEvents(ArrayList<ArrayList<JPDALikelihood>> jointEvents,
				double[][] marginalEvents, ArrayList<JPDALikelihood> singleJointEvent, int objNum) {
		...
		private RealVector updatePrep(SpaceObject currSC, AbsoluteDate tm, int measIndex) {
			...
			for (int i = 0; i < numSigmas; i++) {
				...
				final SpacecraftState[] ssta = new SpacecraftState[1];
				CartesianOrbit orb = new CartesianOrbit(
						new PVCoordinates(new Vector3D(pv[0], pv[1], pv[2]), new Vector3D(pv[3], pv[4], pv[5])),
						odCfg.propInertialFrame, tm, Constants.EGM96_EARTH_MU);
				ssta[0] = new SpacecraftState(orb, odCfg.rsoMass);

				if (singleObject) {
					currSC.estimMeas.setColumn(i,
							odObs.array[measIndex].helpers[0].estimate(0, 0, ssta).getEstimatedValue());
					if (rawMeas == null)
						rawMeas = new ArrayRealVector(odObs.array[measIndex].helpers[0].getObservedValue());
				} else {
					currSC.estimMeas.setEntry(0, i,
							odObs.array[measIndex].helpers[0].estimate(0, 0, ssta).getEstimatedValue()[0]);
					currSC.estimMeas.setEntry(1, i,
							odObs.array[measIndex].helpers[1].estimate(0, 0, ssta).getEstimatedValue()[0]);
					if (rawMeas == null)
						rawMeas = new ArrayRealVector(
								new double[]{odObs.array[measIndex].helpers[0].getObservedValue()[0],
			...
			if (odObs.array[measIndex].station != null) {
				...
				if (pos != null) {
					if (measNames.length == 2) {
						...
						rawMeas = rawMeas.subtract(new ArrayRealVector(
								new double[]{currSC.xhatPrev.getEntry(pos), currSC.xhatPrev.getEntry(pos)}));
					} else
