...
public final class ConversionService extends ConversionGrpc.ConversionImplBase {
	@Override
	public void transformFrame(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
	...
	@Override
	public void convertAzElToRaDec(Messages.AnglesInput req, StreamObserver<Messages.DoubleArray> resp) {
		try {
			double[] raDec = Conversion.convertAzElToRaDec(AbsoluteDate.J2000_EPOCH.shiftedBy(req.getTime(0)),
	...
	@Override
	public void convertRaDecToAzEl(Messages.AnglesInput req, StreamObserver<Messages.DoubleArray> resp) {
		try {
			double[] azEl = Conversion.convertRaDecToAzEl(Predefined.valueOf(req.getFrame()),
					AbsoluteDate.J2000_EPOCH.shiftedBy(req.getTime(0)), req.getAngle1(0), req.getAngle2(0),
	...
	@Override
	public void convertLLAToPos(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
			boolean stringTime = req.getUTCTimeCount() > 0;
			...
			for (int i = 0; i < req.getPvaCount(); i++) {
				...
				Messages.DoubleArray.Builder inner = Messages.DoubleArray.newBuilder().addArray(pos.getX())
						.addArray(pos.getY()).addArray(pos.getZ());
				if (stringTime)
					inner = inner.addArray(time.durationFrom(AbsoluteDate.J2000_EPOCH));
				outer = outer.addArray(inner.build());
			}
	...
	@Override
	public void convertPosToLLA(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
			boolean stringTime = req.getUTCTimeCount() > 0;
			...
			for (int i = 0; i < req.getPvaCount(); i++) {
				...
				GeodeticPoint gp = earth.transform(new Vector3D(pos.getArray(0), pos.getArray(1), pos.getArray(2)),
						srcFrame, time);

				Messages.DoubleArray.Builder inner = Messages.DoubleArray.newBuilder().addArray(gp.getLatitude())
						.addArray(gp.getLongitude()).addArray(gp.getAltitude());
				if (stringTime)
					inner = inner.addArray(time.durationFrom(AbsoluteDate.J2000_EPOCH));
				outer = outer.addArray(inner.build());
			}
	...
	@Override
	public void convertElemToPv(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
			boolean stringTime = req.getUTCTimeCount() > 0;
			Frame srcFrame = FramesFactory.getFrame(Predefined.valueOf(req.getSrcFrame()));
			...
			for (int i = 0; i < req.getPvaCount(); i++) {
				...
				KeplerianOrbit kep = new KeplerianOrbit(pva.get(0), pva.get(1), pva.get(2), pva.get(4), pva.get(3),
						pva.get(5), PositionAngle.values()[pva.get(6).intValue()], srcFrame, time,
						Constants.EGM96_EARTH_MU);
				PVCoordinates pvc = kep.getPVCoordinates();
				double[] pos = pvc.getPosition().toArray();
				double[] vel = pvc.getVelocity().toArray();

				Messages.DoubleArray.Builder inner = Messages.DoubleArray.newBuilder().addArray(pos[0]).addArray(pos[1])
						.addArray(pos[2]).addArray(vel[0]).addArray(vel[1]).addArray(vel[2]);
				if (stringTime)
					inner = inner.addArray(time.durationFrom(AbsoluteDate.J2000_EPOCH));
				outer = outer.addArray(inner.build());
			}
	...
	@Override
	public void convertPvToElem(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
			boolean stringTime = req.getUTCTimeCount() > 0;
			Frame srcFrame = FramesFactory.getFrame(Predefined.valueOf(req.getSrcFrame()));
			...
			for (int i = 0; i < req.getPvaCount(); i++) {
				...
				Messages.DoubleArray.Builder inner = Messages.DoubleArray.newBuilder().addArray(orb.getA())
						.addArray(orb.getE()).addArray(orb.getI())
	...
	@Override
	public void getUTCString(Messages.DoubleArray req, StreamObserver<StringValue> resp) {
		try {
			...
			for (int i = 1; i < req.getArrayCount(); i++) {
				String str = AbsoluteDate.J2000_EPOCH.shiftedBy(req.getArray(i))
						.getComponents(TimeScalesFactory.getUTC()).toString(60, digits);
	...
	@Override
	public void getTTString(Messages.DoubleArray req, StreamObserver<StringValue> resp) {
		try {
			int digits = (int) req.getArray(0);
			ArrayList<String> tt = new ArrayList<String>(req.getArrayCount() - 1);
			for (int i = 1; i < req.getArrayCount(); i++) {
				String str = AbsoluteDate.J2000_EPOCH.shiftedBy(req.getArray(i))
						.getComponents(TimeScalesFactory.getTT()).toString(60, digits);
	...
	@Override
	public void getJ2000EpochOffset(StringValue req, StreamObserver<Messages.DoubleArray> resp) {
		try {
			Messages.DoubleArray.Builder builder = Messages.DoubleArray.newBuilder();
			for (String time : req.getValue().split(" ", 0)) {
				builder = builder
						.addArray(new AbsoluteDate(DateTimeComponents.parseDateTime(time), TimeScalesFactory.getUTC())
	...
	@Override
	public void getMJDEpochOffset(StringValue req, StreamObserver<Messages.DoubleArray> resp) {
		try {
			Messages.DoubleArray.Builder builder = Messages.DoubleArray.newBuilder();
			for (String time : req.getValue().split(" ", 0))
				builder = builder
						.addArray(new AbsoluteDate(DateTimeComponents.parseDateTime(time), TimeScalesFactory.getUTC())
								.durationFrom(AbsoluteDate.MODIFIED_JULIAN_EPOCH));
			resp.onNext(builder.build());
			resp.onCompleted();
		} catch (Throwable exc) {
			resp.onError(new StatusRuntimeException(Status.INTERNAL.withDescription(Tools.getStackTrace(exc))));
	...
	@Override
	public void getUTCStringforMJD(Messages.DoubleArray req, StreamObserver<StringValue> resp) {
		try {
			int digits = (int) req.getArray(0);
			ArrayList<String> utc = new ArrayList<String>(req.getArrayCount() - 1);
			for (int i = 1; i < req.getArrayCount(); i++) {
				String str = AbsoluteDate.MODIFIED_JULIAN_EPOCH.shiftedBy(req.getArray(i))
						.getComponents(TimeScalesFactory.getUTC()).toString(60, digits);
				utc.add(str.substring(0, str.indexOf("+")));
			}
	...
	@Override
	public void getEpochDifference(Messages.IntegerArray req, StreamObserver<DoubleValue> resp) {
		try {
	...
	@Override
	public void transformFrameCov(Messages.TransformFrameInput req, StreamObserver<Messages.Double2DArray> resp) {
		try {
			AbsoluteDate time;
			boolean stringTime = req.getUTCTimeCount() > 0;
			Predefined srcFrame = Predefined.valueOf(req.getSrcFrame());
