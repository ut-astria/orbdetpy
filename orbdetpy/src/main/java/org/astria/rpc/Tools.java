...
public final class Tools {
	public static Settings buildSettingsFromRequest(Messages.Settings req) {
		...
		if (req.getRsoFacetsCount() > 0) {
			...
			for (int i = 0; i < cfg.rsoFacets.length; i++)
				cfg.rsoFacets[i] = new Settings.Facet(
						req.getRsoFacets(i).getNormalList().stream().mapToDouble(Double::doubleValue).toArray(),
		...
		if (req.getRsoSpinAccelerationCount() > 0)
			cfg.rsoSpinAcceleration = req.getRsoSpinAccelerationList().stream().mapToDouble(Double::doubleValue)
					.toArray();
		...
		cfg.dragCoefficient = new Settings.Parameter("Cd", req.getDragCoefficient().getMin(),
				req.getDragCoefficient().getMax(), req.getDragCoefficient().getValue(),
				estmTypes[req.getDragCoefficient().getEstimation()]);
		...
		cfg.rpCoeffReflection = new Settings.Parameter("Cr", req.getRpCoeffReflection().getMin(),
				req.getRpCoeffReflection().getMax(), req.getRpCoeffReflection().getValue(),
				estmTypes[req.getRpCoeffReflection().getEstimation()]);
		...
		if (req.getManeuversCount() > 0) {
			...
			for (int i = 0; i < cfg.cfgManeuvers.length; i++)
				cfg.cfgManeuvers[i] = new Settings.Maneuver(
						AbsoluteDate.J2000_EPOCH.shiftedBy(req.getManeuvers(i).getTime()),
						...
						mnvrVals[req.getManeuvers(i).getManeuverType()], req.getManeuvers(i).getManeuverParamsList()
		...
		if (req.getStationsCount() > 0) {
			...
			for (Map.Entry<String, Messages.Station> kv : req.getStationsMap().entrySet()) {
				...
				cfg.cfgStations.put(kv.getKey(), new Settings.Station(v.getLatitude(), v.getLongitude(),
						v.getAltitude(), v.getBiasList().stream().mapToDouble(Double::doubleValue).toArray(),
		...
		if (req.getMeasurementsCount() > 0) {
			...
			cfg.cfgMeasurements = new HashMap<Settings.MeasurementType, Settings.Measurement>(
					req.getMeasurementsCount());
			for (Map.Entry<Integer, Messages.MeasurementSetting> kv : req.getMeasurementsMap().entrySet()) {
				...
				cfg.cfgMeasurements.put(values[kv.getKey()], new Settings.Measurement(v.getTwoWay(),
		...
		cfg.estmDMCAcceleration = new Settings.Parameter("DMC", req.getEstmDMCAcceleration().getMin(),
				req.getEstmDMCAcceleration().getMax(), req.getEstmDMCAcceleration().getValue(),
				estmTypes[req.getEstmDMCAcceleration().getEstimation()]);
	...
	public static List<Messages.Measurement> buildResponseFromMeasurements(ArrayList<Measurements.Measurement> mlist) {
		...
		for (Measurements.Measurement min : mlist) {
			...
			if (min.angleRates != null)
				builder = builder
						.addAllAngleRates(DoubleStream.of(min.angleRates).boxed().collect(Collectors.toList()));
	...
	public static List<Messages.EstimationOutput> buildResponseFromOrbitDetermination(
			ArrayList<Estimation.EstimationOutput> elist) {
		...
		for (Estimation.EstimationOutput ein : elist) {
			...
			if (ein.propagatedCovariance != null)
				builder = builder.addAllPropagatedCovariance(
						DoubleStream.of(ein.propagatedCovariance).boxed().collect(Collectors.toList()));
			if (ein.innovationCovariance != null)
				builder = builder.addAllInnovationCovariance(
						DoubleStream.of(ein.innovationCovariance).boxed().collect(Collectors.toList()));
			if (ein.estimatedCovariance != null)
				builder = builder.addAllEstimatedCovariance(
						DoubleStream.of(ein.estimatedCovariance).boxed().collect(Collectors.toList()));
			if (ein.preFit != null)
