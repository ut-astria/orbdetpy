...
public final class Settings {
	...
	public static enum AttitudeType {
		UNDEFINED(0), NADIR_POINTING(1), BODY_CENTER_POINTING(2), FIXED_RATE(3);
	...
	public static enum DragModel {
		UNDEFINED(0), EXPONENTIAL(1), MSISE2000(2), WAM(3);
	...
	public static enum EstimationType {
		UNDEFINED(0), CONSIDER(1), ESTIMATE(2);
	...
	public static enum Filter {
		EXTENDED_KALMAN(0), UNSCENTED_KALMAN(1), BATCH_LEAST_SQUARES(2);
	...
	public static enum ManeuverTrigger {
		UNDEFINED(0), DATE_TIME(1), LONGITUDE_CROSSING(2), APSIDE_CROSSING(3), LONGITUDE_EXTREMUM(4), LATITUDE_CROSSING(
				5), LATITUDE_EXTREMUM(6), NODE_CROSSING(7);
	...
	public static enum ManeuverType {
		UNDEFINED(0), CONSTANT_THRUST(1), NORTH_SOUTH_STATIONING(2), EAST_WEST_STATIONING(3), SEMI_MAJOR_AXIS_CHANGE(
				4), PERIGEE_CHANGE(5), ECCENTRICITY_CHANGE(
						6), INCLINATION_CHANGE(7), RAAN_CHANGE(8), ARG_PERIGEE_CHANGE(9), STOP_PROPAGATION(10);
	...
	public static enum MeasurementType {
		AZIMUTH(0), ELEVATION(1), RANGE(2), RANGE_RATE(3), RIGHT_ASCENSION(4), DECLINATION(5), POSITION(
				6), POSITION_VELOCITY(7);
	...
	private void loadForces() {
		...
		if (oceanTidesDegree >= 0 && oceanTidesOrder >= 0)
			forces.add(new OceanTides(FramesFactory.getFrame(Predefined.ITRF_CIO_CONV_2010_ACCURATE_EOP),
					Constants.WGS84_EARTH_EQUATORIAL_RADIUS, Constants.EGM96_EARTH_MU, oceanTidesDegree,
					oceanTidesOrder, IERSConventions.IERS_2010,
		...
		if ((solidTidesSun || solidTidesMoon) && harmonics != null)
			forces.add(new SolidTides(FramesFactory.getFrame(Predefined.ITRF_CIO_CONV_2010_ACCURATE_EOP),
					Constants.WGS84_EARTH_EQUATORIAL_RADIUS, Constants.EGM96_EARTH_MU, harmonics.getTideSystem(),
		...
		if (rsoFacets != null && rsoSolarArrayAxis != null) {
			...
			dragsc = new BoxAndSolarArraySpacecraft(facets, CelestialBodyFactory.getSun(), rsoSolarArrayArea,
					new Vector3D(rsoSolarArrayAxis), dragCoefficient.value, rpCoeffAbsorption, rpCoeffReflection.value);
			radnsc = new BoxAndSolarArraySpacecraft(facets, CelestialBodyFactory.getSun(), rsoSolarArrayArea,
					new Vector3D(rsoSolarArrayAxis), dragCoefficient.value, rpCoeffAbsorption, rpCoeffReflection.value);
		} else {
			dragsc = new IsotropicDrag(rsoArea, dragCoefficient.value);
			radnsc = new IsotropicRadiationSingleCoefficient(rsoArea, rpCoeffReflection.value, rpCoeffReflection.min,
					rpCoeffReflection.max);
		}

		switch (dragModel) {
			case EXPONENTIAL :
				atmModel = new SimpleExponentialAtmosphere(DataManager.earthShape, dragExpRho0, dragExpH0,
						dragExpHscale);
				break;
			case MSISE2000 :
				atmModel = new NRLMSISE00(MSISEInputs.getInstance(), CelestialBodyFactory.getSun(),
						DataManager.earthShape);
		...
		if (cfgManeuvers != null) {
			for (Maneuver m : cfgManeuvers) {
				if (m.triggerEvent == ManeuverTrigger.DATE_TIME && m.maneuverType == ManeuverType.CONSTANT_THRUST)
					forces.add(new ConstantThrustManeuver(m.time, m.maneuverParams[3], m.maneuverParams[4],
							m.maneuverParams[5],
		...
		if (harmonics != null)
			forces.add(new HolmesFeatherstoneAttractionModel(
					FramesFactory.getFrame(Predefined.ITRF_CIO_CONV_2010_ACCURATE_EOP), harmonics));
	...
	private void loadParameters() {
		...
		for (int i = 0; i < ops.length; i++) {
			if (dragCoefficient.estimation == ops[i]) {
				counts[i]++;
				parameters.add(new Parameter(DragSensitive.DRAG_COEFFICIENT, dragCoefficient.min, dragCoefficient.max,
						dragCoefficient.value, ops[i]));
			}
			...
			for (Map.Entry<String, Station> skv : cfgStations.entrySet()) {
				...
				if (sv.biasEstimation == ops[i] && sv.bias != null && sv.bias.length > 0) {
					...
					for (MeasurementType mk : measNames) {
						if (mk == MeasurementType.AZIMUTH || mk == MeasurementType.RANGE
								|| mk == MeasurementType.RIGHT_ASCENSION)
							bias = sv.bias[0];
						else
						...
						parameters.add(
								new Parameter(name, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, bias, ops[i]));
					}
				}
			}
		}
		...
		switch (rsoAttitudeProvider) {
			...
			case FIXED_RATE :
				...
				KeplerianPropagator prop = new KeplerianPropagator(new CartesianOrbit(
						new PVCoordinates(new Vector3D(X0[0], X0[1], X0[2]), new Vector3D(X0[3], X0[4], X0[5])),
	...
	public ArrayList<EventHandling> addEventHandlers(Propagator prop, SpacecraftState initialState) {
		...
		if (geoZoneLatLon != null && geoZoneLatLon.length >= 6) {
			...
			GeographicZoneDetector detector = new GeographicZoneDetector(15.0, 1E-3, DataManager.earthShape, set,
					FastMath.toRadians(0.5));
		...
		if ((outputFlags & Settings.OUTPUT_ECLIPSE) != 0) {
			EclipseDetector detector = new EclipseDetector(CelestialBodyFactory.getSun(), 695700E3,
					DataManager.earthShape).withMaxCheck(15.0).withThreshold(1E-3);
			EventHandler<EclipseDetector> handler = new EventHandling<EclipseDetector>(ManeuverType.UNDEFINED, 0,
					EventHandling.UMBRA, detector.g(initialState) < 0.0);
			prop.addEventDetector(detector.withHandler(handler));
			handles.add((EventHandling) handler);
			detector = detector.withPenumbra();
			handler = new EventHandling<EclipseDetector>(ManeuverType.UNDEFINED, 0, EventHandling.PENUMBRA,
					detector.g(initialState) < 0.0);
			prop.addEventDetector(detector.withHandler(handler));
			handles.add((EventHandling) handler);
		}
		...
		if (cfgStations != null) {
			for (Map.Entry<String, Station> kv : cfgStations.entrySet()) {
				...
				if (stn.fovAperture <= 1E-6) {
					ElevationDetector detector = new ElevationDetector(15.0, 1E-3,
							stations.get(kv.getKey()).getBaseFrame())
					...
					GroundFieldOfViewDetector detector = new GroundFieldOfViewDetector(
							stations.get(kv.getKey()).getBaseFrame(), fov).withMaxCheck(15.0).withThreshold(1E-3);
		...
		for (Maneuver m : cfgManeuvers) {
			...
			switch (m.triggerEvent) {
				case DATE_TIME :
					EventHandler<DateDetector> time = new EventHandling<DateDetector>(m.maneuverType, delta, null,
							false);
				...
				case LONGITUDE_CROSSING :
					EventHandler<LongitudeCrossingDetector> lonc = new EventHandling<LongitudeCrossingDetector>(
							m.maneuverType, delta, null, false);
					prop.addEventDetector(new LongitudeCrossingDetector(DataManager.earthShape, m.triggerParams[0])
							.withHandler(lonc));
					handles.add((EventHandling) lonc);
					break;
				case APSIDE_CROSSING :
					EventHandler<ApsideDetector> apse = new EventHandling<ApsideDetector>(m.maneuverType, delta, null,
							false);
				...
				case LATITUDE_CROSSING :
					...
					prop.addEventDetector(
							new LatitudeCrossingDetector(DataManager.earthShape, m.triggerParams[0]).withHandler(latc));
				...
				case LATITUDE_EXTREMUM :
					...
					break;
				case NODE_CROSSING :
					String type = (m.triggerParams.length == 0 || m.triggerParams[0] == 0.0)
							? EventHandling.ASC_NODE
							: EventHandling.DES_NODE;
					EventHandler<NodeDetector> node = new EventHandling<NodeDetector>(m.maneuverType, delta, type,
							false);
					prop.addEventDetector(
							new NodeDetector(initialState.getOrbit(), propInertialFrame).withHandler(node));
