...
public final class ParallelPropagation {
	...
	private void buildPropagator(Settings cfg) {
		...
		if (cfg.propInitialTLE != null && cfg.propInitialTLE.length == 2) {
			...
			state = new SpacecraftState(new CartesianOrbit(
					new PVCoordinates(new Vector3D(X[0], X[1], X[2]), new Vector3D(X[3], X[4], X[5])),
					cfg.propInertialFrame, cfg.propStart, Constants.EGM96_EARTH_MU), cfg.rsoMass);
			NumericalPropagator np = new NumericalPropagator(new DormandPrince853Integrator(cfg.integMinTimeStep,
					cfg.integMaxTimeStep, cfg.integAbsTolerance, cfg.integRelTolerance));
	...
	private static class Simulation {
		...
		private static String[] biasAzEl = {Settings.MeasurementType.AZIMUTH.name(),
				Settings.MeasurementType.ELEVATION.name()};
		private static String[] biasRaDec = {Settings.MeasurementType.RIGHT_ASCENSION.name(),
		...
		public static boolean simulate(Settings simCfg, SpacecraftState state, ArrayList<EventHandling> handlers,
			...
			if (!simCfg.simMeasurements) {
				...
				for (EventHandling hnd : handlers) {
					if (hnd.detected != null && hnd.observer != null
							&& hnd.observer.equals(EventHandling.GEO_ZONE_NAME)) {
			...
			for (Map.Entry<String, GroundStation> kv : simCfg.stations.entrySet()) {
				...
				for (Map.Entry<Settings.MeasurementType, Settings.Measurement> nvp : simCfg.cfgMeasurements
						.entrySet()) {
					...
					if (name == Settings.MeasurementType.RANGE) {
						...
						obs.addModifier(
								new Bias<Range>(biasRange, new double[]{rand.nextGaussian() * val.error[0] + bias[0]},
						...
						obs.addModifier(new Bias<RangeRate>(biasRangeRate,
								new double[]{rand.nextGaussian() * val.error[0] + bias[bias.length - 1]}, oneOnes,
								oneNegInf, onePosInf));
						if (clone.values == null)
							clone.values = new double[simCfg.cfgMeasurements.size()];
						clone.values[clone.values.length - 1] = obs.estimate(0, 0, ssStates).getEstimatedValue()[0];
					} else if (name == Settings.MeasurementType.RIGHT_ASCENSION
							|| name == Settings.MeasurementType.DECLINATION) {
						AngularRaDec obs = new AngularRaDec(gst, simCfg.propInertialFrame, pvc.getDate(), twoZeros,
								twoZeros, twoOnes, obsSat);
						obs.addModifier(
								new Bias<AngularRaDec>(biasRaDec,
										new double[]{rand.nextGaussian() * val.error[0] + bias[0],
												rand.nextGaussian() * val.error[0] + bias[1]},
										twoOnes, twoNegInf, twoPosInf));
						clone.values = obs.estimate(0, 0, ssStates).getEstimatedValue();
						TimeStampedPVCoordinates gsPvc = gst.getBaseFrame().getPVCoordinates(pvc.getDate(),
								simCfg.propInertialFrame);
						Vector3D gsPos = gsPvc.getPosition(), gsVel = gsPvc.getVelocity();
						TimeStampedPVCoordinates ltSat = pvc
								.shiftedBy(-obs.signalTimeOfFlight(pvc, gsPos, pvc.getDate()));
						Vector3D ltPos = ltSat.getPosition(), ltVel = ltSat.getVelocity();
						double[] dpv = {ltPos.getX() - gsPos.getX(), ltPos.getY() - gsPos.getY(),
								ltPos.getZ() - gsPos.getZ(), ltVel.getX() - gsVel.getX(), ltVel.getY() - gsVel.getY(),
								ltVel.getZ() - gsVel.getZ()};
						double range = Math.sqrt(dpv[0] * dpv[0] + dpv[1] * dpv[1] + dpv[2] * dpv[2]);
						double rrate = (dpv[0] * dpv[3] + dpv[1] * dpv[4] + dpv[2] * dpv[5]) / range;
						clone.angleRates = new double[]{
								(dpv[4] * dpv[0] - dpv[3] * dpv[1]) / (dpv[0] * dpv[0] + dpv[1] * dpv[1]),
						...
						obs.addModifier(
								new AngularRadioRefractionModifier(new EarthITU453AtmosphereRefraction(jsn.altitude)));
						obs.addModifier(
								new Bias<AngularAzEl>(biasAzEl,
										new double[]{rand.nextGaussian() * val.error[0] + bias[0],
												rand.nextGaussian() * val.error[0] + bias[1]},
										twoOnes, twoNegInf, twoPosInf));
						clone.values = obs.estimate(0, 0, ssStates).getEstimatedValue();
						break;
					} else if (name == Settings.MeasurementType.POSITION
							|| name == Settings.MeasurementType.POSITION_VELOCITY) {
