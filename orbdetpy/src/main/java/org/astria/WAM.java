...
public final class WAM implements Atmosphere {
	...
	private static final double[] ATOMIC_MASS = {28.0 * 1.660539E-27, 16.0 * 1.660539E-27, 32.0 * 1.660539E-27}; // N2,
																													// O,
																													// O
				...
				.forEach(cdf -> {
					for (String s : cdf.getFileName().toString().split(Pattern.quote("."))) {
						if (!s.startsWith("20") || s.length() != 15)
							continue;
						s = s.replace("_", "");
						String utc = String.format("%s-%s-%sT%s:%s:%sZ", s.substring(0, 4), s.substring(4, 6),
								s.substring(6, 8), s.substring(8, 10), s.substring(10, 12), s.substring(12, 14));
	...
	public double getDensity(AbsoluteDate date, Vector3D position, Frame frame) {
		...
		synchronized (dataCache) {
			...
			if (entry == null) {
				...
				try (NetcdfFile data = NetcdfFiles.open(finfo.getValue())) {
					...
					entry.density = (float[][][][]) data.findVariable("den")
							.read(String.format("%d,:,1:90,:", timeBlock)).copyToNDJavaArray();
				} catch (Exception exc) {
					throw (new RuntimeException(exc));
				}
			}
		...
		return (interpolator.interpolate(gridX, gridY, gridZ, gridF).value(bound(gridX, lat), bound(gridY, lon),
				bound(gridZ, alt)));
	}

	@Override
	public <T extends CalculusFieldElement<T>> T getDensity(FieldAbsoluteDate<T> date, FieldVector3D<T> position,
			Frame frame) {
