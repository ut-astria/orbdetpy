...
public final class Measurements {
	...
	public Measurements build(Settings odCfg) {
		...
		String[] biasRaDec = {Settings.MeasurementType.RIGHT_ASCENSION.name(),
		...
		for (Measurement m : array) {
			...
			if (cazim != null && celev != null) {
				...
				if (jsn.bias != null && jsn.bias.length > 0)
					obs.addModifier(new Bias<AngularAzEl>(biasAzEl, new double[]{jsn.bias[0], jsn.bias[1]}, twoOnes,
							twoNegInf, twoPosInf));
			}

			if (crigh != null && cdecl != null) {
				AngularRaDec obs = new AngularRaDec(gs, odCfg.propInertialFrame, m.time,
						new double[]{m.values[0], m.values[1]}, new double[]{crigh.error[0], cdecl.error[0]}, twoOnes,
						satellite);
				m.addHelper(obs);
				if (addOutlier)
					obs.addModifier(new OutlierFilter<AngularRaDec>(odCfg.estmOutlierWarmup, odCfg.estmOutlierSigma));
				if (jsn.bias != null && jsn.bias.length > 0)
					obs.addModifier(new Bias<AngularRaDec>(biasRaDec, new double[]{jsn.bias[0], jsn.bias[1]}, twoOnes,
							twoNegInf, twoPosInf));
			}
			...
			if (crang != null) {
				...
				if (jsn.bias != null && jsn.bias.length > 0)
					obs.addModifier(
							new Bias<Range>(biasRange, new double[]{jsn.bias[0]}, oneOnes, oneNegInf, onePosInf));
			}

			if (crrat != null) {
				RangeRate obs = new RangeRate(gs, m.time, m.values[m.values.length - 1], crrat.error[0], 1.0,
						crrat.twoWay, satellite);
				m.addHelper(obs);
				if (addOutlier)
					obs.addModifier(new OutlierFilter<RangeRate>(odCfg.estmOutlierWarmup, odCfg.estmOutlierSigma));
				if (jsn.bias != null && jsn.bias.length > 0)
					obs.addModifier(new Bias<RangeRate>(biasRangeRate, new double[]{jsn.bias[jsn.bias.length - 1]},
							oneOnes, oneNegInf, onePosInf));
			}

			if (cpos != null) {
				Position obs = new Position(m.time, new Vector3D(m.values[0], m.values[1], m.values[2]), cpos.error,
						1.0, satellite);
				m.addHelper(obs);
				if (addOutlier)
