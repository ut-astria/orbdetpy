...
public final class MSISEInputs implements NRLMSISE00InputParameters {
	...
	private MSISEInputs() throws Exception {
		...
		while (scan.hasNextLine()) {
			...
			if (minDate == null)
				minDate = new AbsoluteDate(Integer.parseInt(tok[0]), Integer.parseInt(tok[1]), Integer.parseInt(tok[2]),
						TimeScalesFactory.getUTC());
		}

		scan.close();
		if (tok != null)
			maxDate = new AbsoluteDate(Integer.parseInt(tok[0]), Integer.parseInt(tok[1]), Integer.parseInt(tok[2]),
					TimeScalesFactory.getUTC());
	}
