...
public final class Estimation {
	...
	private class ExtendedKalmanFilter implements CovarianceMatrixProvider, KalmanObserver, OrekitFixedStepHandler {
		...
		private void determineOrbit() {
			...
			final CartesianOrbit orb0 = new CartesianOrbit(
			...
			final KalmanEstimator filter = new KalmanEstimatorBuilder().addPropagationConfiguration(propBuilder, this)
					.build();
		...
		public void evaluationPerformed(KalmanEstimation est) {
			...
			if ((odCfg.outputFlags & Settings.OUTPUT_PROP_COV) != 0) {
				...
				if (estmCovariance != null && phi != null)
					result.propagatedCovariance = getLowerTriangle(
							phi.multiply(estmCovariance).multiply(phi.transpose())
			...
			if (singleObject) {
				...
				if (est.getPhysicalInnovationCovarianceMatrix() != null
						&& (odCfg.outputFlags & Settings.OUTPUT_INNO_COV) != 0)
	...
	private class UnscentedKalmanFilter {
		private void determineOrbit() {
			...
			CartesianOrbit orb0 = new CartesianOrbit(
					new PVCoordinates(new Vector3D(xInitial[0], xInitial[1], xInitial[2]),
							new Vector3D(xInitial[3], xInitial[4], xInitial[5])),
					odCfg.propInertialFrame, odCfg.propStart, Constants.EGM96_EARTH_MU);
			...
			for (int measIndex = 0; measIndex < odObs.array.length; measIndex++) {
				...
				final RealMatrix sqrtP = new CholeskyDecomposition(Ptemp.add(Ptemp.transpose()).scalarMultiply(0.5),
						1E-6, 1E-16).getL();
				...
				for (int attempt = 0; attempt < 2; attempt++) {
					...
					for (int i = 0; i < numSigmas; i++) {
						...
						Pxy = Pxy.add(
								propSigma.getColumnVector(i).subtract(xhatPrev).outerProduct(y).scalarMultiply(weight));
					}
					...
					if ((odCfg.outputFlags & Settings.OUTPUT_RESIDUALS) != 0) {
						...
						if (singleObject)
							...
							odout.postFit = new double[]{
									thisObs.helpers[0].estimate(0, 0, scState).getEstimatedValue()[0],
									thisObs.helpers[1].estimate(0, 0, scState).getEstimatedValue()[0]};
					}

					if (odCfg.estmOutlierSigma > 0.0 && odCfg.estmOutlierWarmup > 0
							&& measIndex >= odCfg.estmOutlierWarmup && attempt == 0 && odout.postFit != null) {
						int pos = 0;
						boolean isOutlier = false;
						double[] fit = odout.postFit;
						for (int j = 0; j < fit.length; j++) {
							if (FastMath.pow(rawMeas.getEntry(pos) - fit[j], 2) > odCfg.estmOutlierSigma
									* odCfg.estmOutlierSigma * Pyy.getEntry(pos, pos)) {
								isOutlier = true;
	...
	private class BatchLeastSquares {
		private void determineOrbit() {
			...
			CartesianOrbit X0 = new CartesianOrbit(
					new PVCoordinates(new Vector3D(x0[0], x0[1], x0[2]), new Vector3D(x0[3], x0[4], x0[5])),
					odCfg.propInertialFrame, odCfg.propStart, Constants.EGM96_EARTH_MU);

			NumericalPropagatorBuilder propBuilder = new NumericalPropagatorBuilder(X0,
					new DormandPrince853IntegratorBuilder(odCfg.integMinTimeStep, odCfg.integMaxTimeStep, 1.0),
