...
obs_list = propagate_orbits(cfg_all)
merge_obs = [sorted((y for x in obs_list for y in list(x.array)), key=lambda x: f"{x.time}{x.station}")]

for cfg in cfg_all:
    cfg.prop_step = 0.0
fit_data = multi_target_OD(cfg_all[:2], merge_obs)

for cfg, obs, fit in zip(cfg_all, fit_data.associated_obs, fit_data.est_output):
    assoc = [merge_obs[0][idx] for idx in obs.array]
    print(assoc[-1].true_state)
    print(fit.array[-1].estimated_state)
